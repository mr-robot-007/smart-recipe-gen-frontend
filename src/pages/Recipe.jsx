...
export default function Recipe() {
    ...
    useEffect(()=> {
        async function fetchRecipes() {
            try {
              setLoading(true);
              const result = await axios.post(`${backendUrl}/recipes/recipe/${id}`,{"user_id":user.id}, {
                headers: {
  ...
  return (
    <>
    {
        ...
        <div className="flex gap-3 items-center text-sm flex-wrap mt-2">
          {recipe?.ingredients?.map((tag, idx) => (
            <span key={idx} className="bg-gray-300 rounded-lg px-2 py-1">
              {tag.ingredient.name} {tag.optional == true ? '(optional)':null} 
            </span>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <p>
          <span className="font-medium">1. Calories:</span> {recipe?.nutri_info}{" "}
        </p>
        <p>
          <span className="font-medium">2. Gluten Free:</span> {recipe?.gluten_free == true ? "✅" : "❌"}{" "}
        </p>
        <p>
          <span className="font-medium">3. Vegetarian:</span> {recipe?.veg == true ? "✅" : "❌"}{" "}
        </p>
        <p>
          <span className="font-medium">4. Cooking Time:</span> {recipe?.nutri_info} mins{" "}
        </p>
        <p>
          <span className="font-medium">5. Difficulty:</span> {recipe?.difficulty}{" "}
        </p>
        <hr />
      </div>
        </div>)
